  ...
  <div class="box">
		...
		<div class="box-body">
			...
			<div>
				...
				<form method=get>
					<input type=hidden value='rekap-total-payroll' name='mn'/>
					...
					<table border=1 class=' table- table-hover' style='width:100%'>
						...
						while($Kar=mysql_fetch_array($query))
						{
							...
							<tr >
								...
								<td><?php echo $lembur_menit = total_lembur_range($idkaryawan,$tgl_baru,$tglakhir) ?></td>
								...
								<td style='text-align:left;font-size:10px'>
									...
									$roll = 2195435;
									$perhari = $roll/30;
									$pot_tidak_masuk = ($tm * $perhari);
									$libur_roll = $total_libur * $perhari;
									$lembur_hasil = (13000*(int)$lembur_menit);
									$premi  = ($tm> 0 ) ? 0 : $perhari;
									$bersih = $roll - $pot_tidak_masuk + $lembur_hasil + $premi - $libur_roll;
									?>
									Gaji : <?php echo rupiah($roll) ?><br/>
									perhari : <?php echo rupiah($perhari) ?><br/>
									tidak masuk <?php echo $tm." x ".rupiah($perhari) ?> : <?php echo rupiah($pot_tidak_masuk) ?><br/>
									lembur <?php echo ($lembur_menit) ?> : <?php echo rupiah($lembur_hasil) ?><br/>
									libur  : <?php echo rupiah($libur_roll) ?><br/>
									premi  : <?php echo rupiah($premi) ?><br/>
									total :   <?php echo rupiah($bersih) ?><br/>
								</td>
							</tr>
						<?php 
						$no++;
